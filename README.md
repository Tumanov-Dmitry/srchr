...
Административная документация доступна по адресу `/admin/knowledge`.

- `docs/modules/index.json` хранит компактный индекс, статусы и зависимости.
- `docs/modules/*.md` описывают бизнес-логику и техническую реализацию модулей.
- При изменении функциональности обновляйте только затронутый документ и поле
  `updatedAt` в индексе.

Документация хранится в Git и не требует отдельной таблицы Supabase.
